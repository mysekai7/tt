...
//js根目录
var root = jsLib + "popMenu/";
//数据文件目录
var dataDir = jsRoot + "data/";
//php调用文件路径
...
{
	new popMenu(btnObj,target,app,title,deep,trigger,url,sbmtUrl,callFunc);
}
/**
* 定义对象的方法
* 
*/
var popMenu = function() { this.initialize.apply(this, arguments); }; //等价于执行了： popMenu
popMenu.prototype = {
		...
		self.panel.initNext = function(ele,nFile) {
			...
			if(typeof ele != "undefined") {
				...
				if(self.panel.url && self.panel.url.length>1) {
					...
					if(!nFile)
						_url = dataDir+self.app+"/0.js";
					else
						_url = dataDir+self.app+"/"+nFile;
				}
		...
		self.panel.assignNext = function(ele) {
			...
			if(typeof ele.value != "undefined") {
				if(self.panel.url && self.panel.url.length>1) {
					...
					_url = dataDir+self.app+"/"+el.value+".js";
