...
$lang = array (
  ...
  'api_doc_cat_1_post_2_text' => 'Not all functionalities offered by our API require authentication. Functionalities with free access are those that everyone can access freely and this do not include modification options like deleting, editing, adding of data and reading of personal data of our users. The API has a few methods that would require the user to authenticate in order to access and/or publish data. For instance, functionality on [b]#SITE_TITLE#[/b] available to logged in users, such as submitting updates or downloading direct messages, are similarly protected through the API. In order to protect the data of our clients we use the OAuth protocol for authenticating the application\'s access to the data of our users.',
  'api_doc_cat_1_post_3_title' => 'OAuth',
  'api_doc_cat_1_post_3_text' => '[url=http://apiwiki.twitter.com/Authentication]http://apiwiki.twitter.com/Authentication[/url] OAuth is a token-passing mechanism that allows users to control which applications have access to their data without giving away their passwords. More information on the OAuth specification can be found at [url=http://www.oauth.net]oauth.net[/url] or in the excellent Beginner\'s Guide to OAuth from [url=http://hueniverse.com/oauth/]Hueniverse[/url]. Registration for OAuth applications for [b]#SITE_TITLE#[/b] is available at [url=#SITE_URL#api]#SITE_URL#api[/url]. Access tokens should never expire. An user\'s access token will be invalidated if the user explicitly revokes your application\'s authorization or if [b]#SITE_TITLE#[/b] suspends your application. If your application is suspended there will be a note on your application page saying that it has been suspended. In either case, your application should be able to handle token revokation gracefully. Many users trust an application to read their information but not necessarily change their name or post new statuses. OAuth permissions are granular, allowing applications and users to agree on Read-Only or Read-Write access. Updating information through the API, be it name, location or adding updating status, requires an HTTP POST. Any API method that requires an HTTP POST is considered a write method and requires Read-Write access.',
