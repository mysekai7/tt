<?php
	...
	class newpost
	{
		...
		private $posttags_container	= array();
		private $poststags_orig = 0;
		private $db1;
		private $db2;
		private $cache;
			...
			if( ! $this->id )
			{
				...
				if( ! $is_private ) {
					...
					if( $this->user->id > 0 && count($this->mentioned) > 0 ) {
						...
						if( !$this->group || $this->group->is_public ) {
							foreach($this->mentioned as $uid) {
								if( $uid == $this->user->id ) {
									continue;
								}elseif($this->user->is_posts_protected && !isset($this->network->get_user_follows($this->user->id, FALSE, 'hefollows')->follow_users[$uid]) && !$this->network->get_user_by_id($uid)->is_network_admin && !$this->group){
									continue;
								}
								$notify[]	= $uid;
							}
						...
						foreach($notify as $uid) {
							...
							if( $n==1 || $n==3 ) {
								...
								$message_txt	= $page->lang('emltxt_ntf_me_if_u_posts_qme', $lng_txt, $ulng).$this->represent_post_in_email(false);
								$message_htm	= $page->lang('emlhtml_ntf_me_if_u_posts_qme', $lng_htm, $ulng).$this->represent_post_in_email();
					...
					if( $n==1 || $n==3 ) {
						...
						$message_txt	= $page->lang('emltxt_ntf_me_if_u_posts_prvmsg', $lng_txt, $ulng).$this->represent_post_in_email(false);
						$message_htm	= $page->lang('emlhtml_ntf_me_if_u_posts_prvmsg', $lng_htm, $ulng).$this->represent_post_in_email();
