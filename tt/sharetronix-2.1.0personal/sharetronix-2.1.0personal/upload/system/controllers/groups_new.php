				...
				if( $n == 1 ) {
					...
					foreach($followers as $uid) {
						...
						if( $send_post ) {
							...
							$this->network->send_notification_post($uid, 0, 'msg_ntf_me_if_u_creates_grp', $lng, $C->NOTIF_POSTS_HANDLE);
						}
						if( $send_mail ) {
							...
							$subject		= $this->lang('emlsubj_ntf_me_if_u_creates_grp', $lng_txt, $ulng);
							$message_txt	= $this->lang('emltxt_ntf_me_if_u_creates_grp', $lng_txt, $ulng);
							$message_htm	= $this->lang('emlhtml_ntf_me_if_u_creates_grp', $lng_htm, $ulng);
