...
class mysql
{
    ...
    public $fatal_error = true;
    private $debug_mode = true;
    private $debug_info = true;
    private $ext;
    public $disconnect_on_desctruct;
    ...
    public function __construct($host, $user, $pass, $db)
    {
        ...
        $this->dbpass = $pass;
        $this->dbname = $db;
        $this->debug_mode = $C->DEBUG_MODE;
        if( !isset($C->MYSQL_DEBUG_INFO) )
        {
            $C->MYSQL_DEBUG_INFO = (object)array('queries'=>array(), 'time'=>0);
        }
        $this->debug_info = &$C->MYSQL_DEBUG_INFO;

        $this->ext = 'mysql';

        if(isset($C) && isset($C->DB_MYEXT) && $C->DB_MYEXT == 'mysqli' && function_exists('mysqli_connect'))
        {
            $this->ext = 'mysqli';
        }
        $this->disconnect_on_desctruct = TRUE;
    }
    ...
    {
        $res = $this->ext == 'mysqli' ? $res : $this->last_result;
        if(FALSE == $res)
        {
            return FALSE;
        }
    ...
    {
        $res = $res == FALSE ? $res : $this->last_result;
        if(FALSE == $res)
        {
            return $res;
        }
    ...
    {
        if($this->disconnect_on_desctruct)
        {
            if($this->connection)
            {
