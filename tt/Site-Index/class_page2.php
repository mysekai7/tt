...
class page2 {
    ...
    public function getPage() {
        ...
        if($prevPage > 0)
        {
            if($prevPage==1)
            {
                $html_page .= "<a href='{$this->Url}{$this->Condition}'>Previous</a> ";
            }
            else
            {
                $html_page .= "<a href='{$this->Url}_$prevPage{$this->Condition}'>Previous</a> ";
            }
        }
        //前一波段
        if($prevArea > 0){
            if($prevArea == 1)
            {
                $html_page .= "<a href='{$this->Url}{$this->Condition}'>1</a> ";
                $html_page .= "<a href='{$this->Url}{$this->Condition}'>...</a> ";
            }
            else
            {
                $html_page .= "<a href='{$this->Url}{$this->Condition}'>1</a> ";
                $html_page .= "<a href='{$this->Url}_$prevArea{$this->Condition}'>...</a> ";
            }
        }
        ...
        for($i = $startPage; $i < $startPage + $area; $i++)
        {
            ...
            else
            {
                if($i == 1)
                {
                    $html_page .= "<a href='{$this->Url}{$this->Condition}'>{$i}</a> ";
                }
                else
                {
                    $html_page .= "<a href='{$this->Url}_$i{$this->Condition}'>{$i}</a> ";
                }
            }
        }
        ...
        //下一波段
        if($nextArea <= $maxPage)
            $html_page .= "<a href='{$this->Url}_$nextArea{$this->Condition}'>...</a> ";

        //最后一页
        if($startPage + $area > 4 && $nextArea <= $maxPage)
            $html_page .= "<a href='{$this->Url}_$maxPage{$this->Condition}'>$maxPage</a> ";

        //下一页
        if($nextPage <= $maxPage)
            $html_page .= "<a href='{$this->Url}_$nextPage{$this->Condition}'>Next</a> ";
